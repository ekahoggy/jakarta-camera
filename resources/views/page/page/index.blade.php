...
@section('content')
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        <label class="custom-switch" style="cursor: pointer">
                                            <input type="checkbox" name="option" id="publish{{ $val->id }}"
                                                class="custom-switch-input" @if ($val->is_status == 2) checked @endif
                                    ...
                                    <td class="align-middle">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <!--
                                                <button class="btn btn-sm btn-danger fa fa-trash mr-2"
                                                    onclick="hapus({{ $val->id }})"></button>
                                            -->
                                            <button class="btn btn-sm btn-warning fa fa-copy"
                                                onclick="duplicate({{ $val->id }})"></button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
    ...
    <script>
        ...
        function duplicate(id) {
            Swal.fire({
                title: 'Confirmation',
                text: 'Are you sure you want to duplicate this data?',
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    axios.post(`/page/duplicate/${id}`).then(res => {
                            if (res.data.success) {
                                Swal.fire('Success', 'Duplicate Success', 'success').then(() => {
                                    location.reload();
                                });
                            } else {
                                Swal.fire('Ops..', 'There is an error.', 'error');
